...
trials_10mon <- trial_tidy_all %>%
  ...
  filter( # Remove rows with filled NAs in vars_10mon
    !if_all(
      .cols = vars_10mon,
      .fns = ~ is.na(.x)
    )
...
```{r eval=FALSE}
write.csv(my_dat, here::here("output", paste("01_", year_interest, trial_interest, "_tidy_data4analysis_", Sys.Date(), ".csv", sep = "")), row.names = FALSE)
```
...
# I've removed "plant_type", "branch_number", "root_type1_5" , "height_1st_branch" wild crossing
# because not were fitted by the model.
# DM_gravity was remove because is redundant with DM_oven


if (trial_interest == "PDGN1") {
  trait_ideal_pivot <- trait_ideal_pivot[-c(2, 3, 5, 7, 8)]
}
...
meta_file_name <- paste0(folder_output, paste0("2022", trial_interest, "_master_results_", Sys.Date(), ".xlsx"))
...
```{r}
colnames(blupDF_kp) = c("accession_name", "Cooking time (min)", "Dry matter by oven (%)", "Cyanide content (ppm)", "Plant height (cm)", "PPD", "Yield (t/ha)")
...
```{r}
index_col <- c(
  "PPD", "Cooking time (min)", "Dry matter by oven (%)", "Cyanide content (ppm)",
  "Yield (t/ha)"
)
...
# PCA
pcs <- index_dat %>%
  dplyr::select(-c("Cooking time (min)")) %>%
  drop_na()
...
```{r}
# IND
IND <- fviz_pca_ind(pca, repel = T, alpha.ind = 0.3, col.ind = "grey20", labelsize = 2) +
...
BLUEs_BLUPs <- 
  ...
  select(accession_name, index, selected, everything())
...
#cluster tree
hcpc <- HCPC(pca, nb.clust = 0, method = "ward", graph = FALSE)
...
The next plot is the clustering plot, but this time in the space of the principal components.

```{r message=FALSE, warning=FALSE}
hcpc_pca <- fviz_cluster(hcpc,
             repel = TRUE,
...
ggsave(paste("images/cluster",  trial_interest, Sys.Date(), ".png", sep = "_"), plot = hcpc_pca, units = "in", dpi = 300, width = 10, height = 10)
```
...
# Asignacion de cluster a cada accession_name


```{r}
cluster_asignados <- hcpc$data.clust

cluster_asignados <- cluster_asignados %>% rownames_to_column("accession_name") %>% select(accession_name, clust, everything())

BLUEs_BLUPs <- 
  master_data$BLUEs_BLUPs_MET %>% 
  left_join(cluster_asignados[c(1:2)], by = c("accession_name" = "accession_name")) %>% 
  select(accession_name, clust, everything())
```
