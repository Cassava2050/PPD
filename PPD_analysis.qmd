...
# There are yield errors 

trial_tidy_all <- trial_tidy_all %>%
    mutate(
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM3732-5_2", NA, yield_ha), 
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM3736-21_22", NA, yield_ha),
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM3736-26_24", NA, yield_ha),
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM3736-51_33", NA, yield_ha),
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM905-52_41", NA, yield_ha),
      yield_ha = ifelse(plot_name == "201680BCGN1_ciat_rep1_GM905-57_12", NA, yield_ha)) 
```
...
PPD <- PPD %>%
  ...
  ) %>%  # paste letter "S" to the slice column, remove repeat columns and 
  mutate(slice = paste0("s", slice)) %>%
  select(-c(plot_name, plant)) %>%
  rename_all(tolower) # make all colname tolower
...
# LAB
PPD_Becerra <- PPD %>% 
  filter(slice %in% c("slice3", "slice4", "slice5")) %>% 
...
# ggsave(paste0("images\\boxplot_", trial_interest, "_", Sys.Date(), ".png"),
...
trait_ideal_pivot <- trait_ideal_pivot[!trait_ideal_pivot %in% c("DM_gravity", 
                                                                "plant_type",
                                                                "branch_number",
                                                                "root_type1_5",
                                                                "cook_time")]
...
for (var in trait_ideal_pivot) {
  ...
  model_MET_2stage <- asreml(
    fixed = eqt_fixed,
    random = ~ fa(trial_name, 1):accession_name,
    data = my_dat %>% filter(trial_name %in% trials) %>% droplevels(),
    family = asr_gaussian(dispersion = 1),
    na.action = list(x = "exclude", y = "exclude"),
    trace = 0
  ...
  h2_fa1[i] <- agriutilities::heritability_fa(
    model_fa = model_MET_2stage,
    genotype = "accession_name",
    env = "trial_name",
    vc_model = "fa1"
...
vc <- summary(model_MET_2stage)$varcomp
(R<-vc[1:10,1])
(L<-vc[11:20,1])
(V<-L%*%t(L)+diag(R))
G.fa <- cov2cor(V)
...
## Cook time trait
Cook time trait has very low heritability, I need to fix this data checking the raw data. After checking the different trials across years, I decided to remove Cooking time from the study. The correlations between the trials were so low. 

```{r}
my_dat %>% select(trial_name)

var = "cook_time"
cook_time_spread <- my_dat %>% 
...
# remove empty cols
  ...
  cook_time_spread <- cook_time_spread[, colSums(is.na(cook_time_spread)) < nrow(cook_time_spread)]
  
  
M <- cor(as.matrix(cook_time_spread[, -1]), use = "pairwise.complete.obs")
...
```{r}
h2_fa1 <- as.data.frame(do.call(cbind, h2_fa1)) %>% 
  pivot_longer(everything(), names_to = "trait", values_to = "h2")

# save the heritability
master_data[[paste0("h2_", trial_interest)]] <- h2_fa1
```
...
BLUEs <- my_dat %>% select(trial_name, accession_name, trait_ideal_pivot) %>% 
  pivot_wider(names_from = trial_name, values_from = trait_ideal_pivot)

colnames(BLUEs)[-1] <- paste("BLUEs", colnames(BLUEs)[-1], sep = "_")

BLUEs_BLUPs <- BLUEs %>% 
  left_join(pvals_table %>%
  select(-c(status)) %>%
  group_by(variable, accession_name) %>% 
      rename(
      ...
      pivot_wider(names_from = "variable", values_from = c("BLUPs", "seBLUPs")),
    by = "accession_name"
  ) %>% 
  arrange(desc(BLUPs_DM_raw)) %>% 
...
for (i in 1:length(trait_ideal_pivot)) {
  ...
  header_sort = c(header_sort, 
                  grep(trait_ideal_pivot[i], sort(names(BLUEs_BLUPs)), value=TRUE) 
  )
  
}

BLUEs_BLUPs <- BLUEs_BLUPs %>%
  select(accession_name, all_of(header_sort)) 


BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  relocate(colnames(BLUEs_BLUPs)[str_detect(colnames(BLUEs_BLUPs), "DM_raw")], .after = accession_name)
...
names(master_data)[11] <- "BLUP_BLUE__caroteno_BCGN1" #"BLUP_BLUE__bcaroteno_BCGN1"
names(master_data)[10] <- "BLUP_BLUE__bcaroteno_BCGN1"
...
colnames(blupDF_kp) <- c("accession_name", "B-carotene by NIR (ug/g)", "Total carotenoids by NIR (ug/g)", "Dry matter by oven (%)", "Cyanide content (ppm)", "Plant height (cm)", "1st branch height (cm)", "PPD", "Yield (t/ha)")

geno_cor <- gg_cor(
  colours = c("red", "white", "blue"),
  data = blupDF_kp, # remove carotenoids
  label_size = 4
) 
...
index_col <- c(
  "PPD", "Dry matter by oven (%)", "Yield (t/ha)", "1st branch height (cm)"
)

index_dat <- blupDF_kp %>%
  dplyr::select("accession_name", index_col) %>%
  column_to_rownames("accession_name")
```

## Index selection function

Include this function on utilities and make a package with utilities
...
res.pca <- pca_index(data = TestIndex_, id = "accession_name", b = c(-10, 10, 10, -10), percentage = 0.20)
...
```{r}
BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  left_join(selections[-c(2:5)], by = c("accession_name" = "accession_name")) %>% 
